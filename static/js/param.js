...
function renderCategoryList() {
    ...
    const totalPages = Math.ceil(categories.length / PAGE_SIZE.category);
    const currentPage = currentPages.category;
    const start = currentPage * PAGE_SIZE.category;
    const end = start + PAGE_SIZE.category;
    const pageCategories = categories.slice(start, end);

    // 改为2列布局
    let html = '<div class="row g-2">';
    html += pageCategories.map((category, index) => `
        <div class="col-6">
            <button type="button" 
                    class="btn btn-outline-secondary w-100 btn-category-item ${index === 0 && currentPage === 0 ? 'active' : ''}"
    ...
    // 绑定分类点击事件
    categoryListEl.querySelectorAll('.btn-category-item').forEach(btn => {
        btn.addEventListener('click', (e) => {
            categoryListEl.querySelectorAll('.btn-category-item').forEach(b =>
                b.classList.remove('active'));
            e.currentTarget.classList.add('active');
            currentSelection.category = e.currentTarget.dataset.category;
            currentPages.chapter = 0;
            renderChapterList();
        });
    });
...
function renderChapterList() {
    ...
    const totalPages = Math.ceil(chapters.length / PAGE_SIZE.chapter);
    const currentPage = currentPages.chapter;
    const start = currentPage * PAGE_SIZE.chapter;
    const end = start + PAGE_SIZE.chapter;
    const pageChapters = chapters.slice(start, end);

    // 改为3列布局
    let html = `<h6 class="mb-2">商品章节</h6><div class="row g-2">`;
    html += pageChapters.map((chapter, index) => `
        <div class="col-4">
            <button type="button" 
                    class="btn btn-outline-secondary w-100 btn-chapter-item ${index === 0 && currentPage === 0 ? 'active' : ''}"
                    data-chapter="${chapter}"
                    title="${chapter}">
                ${truncateText(chapter, 8)}
            </button>
        </div>
    `).join('');
    html += '</div>';
    ...
    // 绑定章节点击事件
    chapterListEl.querySelectorAll('.btn-chapter-item').forEach(btn => {
        btn.addEventListener('click', (e) => {
            chapterListEl.querySelectorAll('.btn-chapter-item').forEach(b =>
                b.classList.remove('active'));
            e.currentTarget.classList.add('active');
            currentSelection.chapter = e.currentTarget.dataset.chapter;
            currentPages.product = 0;
            renderProductList();
        });
    });
...
function renderProductList() {
    ...
    const totalPages = Math.ceil(products.length / PAGE_SIZE.product);
    const currentPage = currentPages.product;
    const start = currentPage * PAGE_SIZE.product;
    const end = start + PAGE_SIZE.product;
    ...
    html += pageProducts.map(product => `
        <div class="col-2">
            <button type="button" 
                    class="btn btn-outline-primary w-100 btn-product-item"
                    data-product="${product}"
                    title="${product}">
                ${truncateText(product, 8)}
            </button>
        </div>
    `).join('');
    html += `</div>`;
    ...
    function renderProvinceSelect() {
        ...
        const provinces = apiData.provinces;
